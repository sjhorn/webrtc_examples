...
void main() async {
  ...
  print('Loaded audio: ${audioPackets.length} pkts, video: ${videoPackets.length} pkts (${duration.toStringAsFixed(2)}s) [UNBUNDLED]');
...
Future<void> handleWebSocket(HttpRequest request, List<Uint8List> audioPackets,
  ...
  final pc = RTCPeerConnection(RtcConfiguration(
    iceServers: [],
    bundlePolicy: BundlePolicy.disable,  // True unbundled - separate transports
    codecs: RtcCodecs(
  ...
  pc.onConnectionStateChange.listen((s) {
    ...
    if (s == PeerConnectionState.connected) connected = true;
  });

  pc.onIceConnectionStateChange.listen((s) {
    print('ICE connection state: $s');
  });

  pc.onIceGatheringStateChange.listen((s) {
    print('ICE gathering state: $s');
  });
  ...
  socket.listen((data) async {
    ...
    if (m['type'] == 'answer') {
      ...
      print(m['sdp']);
      print('==================');
      await pc.setRemoteDescription(RTCSessionDescription(type: 'answer', sdp: m['sdp'] as String));
      print('Set remote description, connection state: ${pc.connectionState}');
    }
