...
(async () => {
  ...
  // Click play (don't await - srcObject reset in 'started' handler will abort it)
  ...
  // Verify video is actually rendering (videoWidth > 0 means frames received)
  await page.waitForFunction(() => {
  ...
  // Verify video is actually rendering during replay
  await page.waitForFunction(() => {
    const v = document.querySelector('video');
    return v.videoWidth > 0 && v.videoHeight > 0;
  }, { timeout: 5000 });
